# -*- coding: utf-8 -*-
"""
Autodesk Inventor COM API 기반 자동 채점 스크립트 (grade_june15_pc13.py)
- PC-10 폴더를 기준(Reference)으로 삼아 PC-13 폴더의 제출물을 정밀 검사합니다.
- 주요 검사 항목:
  1. 파일명 규칙 위반 여부 (언더바 _ 가 정답 규칙)
  2. 부품 간 간섭 검사
  3. 조립 파일 링크 유실 여부
  4. 기준(PC-10) 대비 단품 체적 오차 검사 (허용 오차 5% 이내)
"""
...
REF_DIR = r"D:\이갑종\26 3학년 전일제\6월15 TEST\PC-10_10.128.56.100"
...
def run_grading():
    ...
    # 1단계: 기준 폴더(PC-10)에서 정답 체적 데이터 추출
    print("\n--- [1단계] 기준 폴더(PC-10) 정답 체적 데이터 빌드 중 ---")
    ref_database = {}
    ...
    # 2단계: 대상 폴더(PC-13) 채점 진행
    print("\n--- [2단계] 대상 폴더(PC-13) 채점 진행 중 ---")
    results = {}
    ...
    # 3단계: 보고서 작성 (PC-13 폴더 아래에 생성)
    report_path = os.path.join(TARGET_DIR, "grade_report_june15.md")
    
    md = []
    md.append(f"# 3D 프린터 운용기능사 실기 시험 자동 채점 보고서 (PC-13)")
    md.append(f"- **채점 일시**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    md.append(f"- **기준 폴더(PC-10)**: `{REF_DIR}`")
    md.append(f"- **대상 폴더(PC-13)**: `{TARGET_DIR}`")
    ...
    for folder in subdirs:
        ...
        if assembly and assembly.get("success"):
            ...
            # 체적 비교 출력
            md.append("\n**📐 체적(Volume) 비교 분석 (vs PC-10 기준)**")
            md.append("| 부품 | 기준 체적 (PC-10) | 학생 체적 (PC-13) | 오차 비율 | 상태 |")
            md.append("| :--- | :---: | :---: | :---: | :---: |")
    ...
    print("\n" + "="*75)
    print(" 6월 15일 TEST PC-13 채점 완료 (기준: PC-10)")
    print("="*75)
